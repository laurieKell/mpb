...
GLOBALS_SECTION
  #include "admodel.h"
  #include <string>
  #include <dnorm.hpp> //include functions from custom library
  #include <nr.hpp>    //include functions from custom library
...
REPORT_SECTION
  ...
  for (int i=1; i<=ni; i++){
    ll[Idx[i]] += log(_s[Idx[i]])+  
             pow(log(I[i])-log(Ifit[i]),2.0)/(2*_s[Idx[i]]*_s[Idx[i]]);
  //  rss += pow(log(I[i])-log(Ifit[i]),2.0);
