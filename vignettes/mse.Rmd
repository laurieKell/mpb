...
plot(srDev)+
    geom_line(aes(year,data,col=iter),data=as.data.frame(iter(srDev,1:3)))
```
...
plot(oms[["Projection"]])+
  geom_line(aes(year,data,col=iter),
            data=as.data.frame(FLQuants(iter(oms[["Projection"]],1:3),"Rec"=rec,"F"=fbar,"SSB"=ssb,"Catch"=catch),drop=TRUE))+
...
plot(oms[["Age"]])+
  geom_line(aes(year,data,col=iter),
            data=as.data.frame(FLQuants(iter(oms[["Age"]],1:3),"Rec"=rec,"F"=fbar,"SSB"=ssb,"Catch"=catch),drop=TRUE))+
  theme(legend.position="none")
```
...
```{r biodyn-xtest-plot}
dat=plot(FLQuants(mp,"Biomass"=stock,"F"=function(x) catch(x)/stock(x)[,dimnames(catch(x))$year]))$data

plot(FLQuants(window(oms[["Projection"]],end=75),"Biomass"=stock,"F"=function(x) catch(x)/stock(x)))+
  geom_line(  aes(year,`50%`),data=dat)+
  geom_ribbon(aes(year,ymax=`75%`,ymin=`25%`),data=dat,alpha=.5)
```
...
plot(window(oms[["Biomass"]],end=100))+
  geom_line(aes(year,data,col=iter),
            data=as.data.frame(FLQuants(window(iter(oms[["Biomass"]],1:3),end=100),"Rec"=rec,"F"=fbar,"SSB"=ssb,"Catch"=catch),drop=TRUE))+
  theme(legend.position="none")
```
**Figure `r iFig=iFig+1; iFig`** Time series from the MSE of F, SSB, recruitment and yield
...
plot(window(oms[["Emprirical"]],end=100))+
  geom_line(aes(year,data,col=iter),
            data=as.data.frame(FLQuants(iter(window(oms[["Emprirical"]],end=100),1:3),"Rec"=rec,"F"=fbar,"SSB"=ssb,"Catch"=catch),drop=TRUE))+
  theme(legend.position="none")
```
