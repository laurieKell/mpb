...
```{r prfl, fig.margin=TRUE, eval=FALSE,fig.height=3, fig.cap="Likelihood profile for r"}
bdHat=fit(bdHat,cpue)
setControl(bdHat)=params(bdHat)
res=profile(bdHat,which='r',fixed=c('b0','p'),cpue,range=seq(0.95,1.03,.002))
...
```{r uncertaintyBoot, eval=FALSE,fig.height=4, fig.margin=TRUE, fig.cap="Bootstrapped CPUE series"}
set.seed(7890)
cpueBoot      =boot(bdHat)
...
```{r uncertaintyJackknife, eval=FALSE, fig.height=4,fig.margin=TRUE, fig.cap="Plot predicted stock trend by index"}
...
```{r refBmsy,eval=FALSE,fig.margin=TRUE, fig.width=4, fig.height=3,fig.cap="Densities of Stock from different methods for estimating uncertainty."}
...
```{r hcrPlot,eval=FALSE, fig.margin=TRUE,fig.width=4, fig.height=4,fig.cap="Plots of projections"}
plot(simHCR)+
  theme_bw()+
  theme(legend.position="bottom")
```
...
```{r MC,eval=FALSE,fig.margin=TRUE,fig.width=6,fig.height=6}
set.seed(7890)
