...
sigma=aaply(FLPar(c(sigma),dimnames=dimnames(n)[c(1,1:6)]),3:7,fumction(x) x%*%l)

upper=n%=%20

x=rtmvnorm(n=max(n[,1]), mean=c(l[,1]), sigma=matrix(c(sigma@.Data[,,1,1,1,1,1]),10,10), upper=c(upper[,1]))
x=melt(x)
...
lem2<-function(n,l,sd){
  ...
  ln=apply(l%*%u,2:6,sum)%/%apply(u,2:6,sum)
