...
Type objective_function<Type>::operator() () {
  ...
  Type r=par(0);
  if(ctl(0,0)<=0) r =ctl(0,2); else r =invLogit( r, ctl(0,1),ctl(0,3));
  Type k=par(1);
  if(ctl(1,0)<=0) k =ctl(1,2); else k =invLogit( k, ctl(1,1),ctl(1,3));
  Type p=par(2);
  if(ctl(2,0)<=0) p =ctl(2,2); else p =invLogit(p ,ctl(2,1),ctl(2,3));
  Type b0=par(3);
  if(ctl(3,0)<=0) b0=ctl(3,2); else b0=invLogit(b0,ctl(3,1),ctl(3,3));
